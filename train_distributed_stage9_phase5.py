...
def main():
    ...
    phase5b_results = phase5a_best.train(
        ...
        cache="disk",                 # 磁盤緩存
        optimizer="AdamW",            # AdamW優化器
        lr0=0.0001,                   # 降低學習率以提高穩定性
        lrf=0.00001,                  # 極低最終學習率
        momentum=0.937,               # 標準動量
        weight_decay=0.0001,          # 增加權重衰減以改善正則化
        warmup_epochs=5.0,            # 適度熱身
        box=1.0,                      # 最低框損失權重
        pose=150.0,                   # 降低姿態損失權重以避免過度擬合
        kobj=60.0,                    # 降低關鍵點對象損失權重
        cls=0.01,                     # 增加分類損失權重
        dfl=0.05,                     # 增加分布焦點損失權重
        cos_lr=True,                  # 餘弦學習率調度
        amp=True,                     # 混合精度訓練
        ...
        val=True,                     # 驗證
        plots=True,                   # 圖表
        label_smoothing=0.05,         # 減少標籤平滑
        multi_scale=False,            # 關閉多尺度訓練
        rect=True,                    # 使用矩形訓練以提高穩定性
        ...
        project="runs/pose",          # 項目名稱
        name="stage9_phase5b_stable", # 修改訓練名稱
        exist_ok=True                 # 覆蓋已有目錄
    )
    
    # 獲取階段5B最佳權重
    phase5b_best = YOLO("runs/pose/stage9_phase5b_stable/weights/best.pt")
